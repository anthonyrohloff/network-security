from .setup_functions import _generate_4x4_matrix

def hex_convert(input_array):
    output_array = []
...
def _add_key(text, subkey):
    ...
    print(f"AddKey: {hex_convert(output_array)}")
    return output_array
...
def _sub_bytes(text):
    ...
    print(f"SubBytes: {hex_convert(output_array)}")
    return output_array
...
def _shift_rows(text):
    ...
    print("HERE", hex_convert(text))
    cols_to_rows = []
    for i in range(4):
        cols_to_rows.append([text[0][i], text[1][i], text[2][i], text[3][i]])
    
    print(hex_convert(cols_to_rows))
    # row 0 does not get shifted, so append it manually
    output_array.append(cols_to_rows[0])

    index = 1
    while index < len(cols_to_rows):
        output_array += [cols_to_rows[index][index:] + cols_to_rows[index][:index]]
        index += 1

    print(f"ShiftRows: {hex_convert(output_array)}")
    return output_array


def _mix_columns(text):
    mix_cols_constant = [[2, 3, 1, 1],
                         [1, 2, 3, 1],
                         [1, 1, 2, 3],
                         [3, 1, 1, 2]]
    ...
    for i in range(4):
        for j in range(4):
            for k in range(4):
                result[i][j] += text[i][k] * mix_cols_constant[k][j]

    print(f"MixColumns: {hex_convert(result)}")
